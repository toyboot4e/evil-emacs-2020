...
image::neotree.png[]
+
{neotree} is an alternative to NERDTree in Vim. {neotree} is easy to use, but {treemacs} supports multiple root directories, so I'd try {treemacs} later (it's not included in this repo).
...
+
Alternatives to olivetti would be https://github.com/joaotavora/darkroom[darmroom] and https://github.com/joostkremers/writeroom-mode[writeroom-mode].
...
I didn't use `after-init-hook` because I didn't think it made sense. `after-init-hook` is run after `emacs-init-time`, so `after-init-hook` pretends faster startup, which is obviously not a good thing. Just my curious, but is there any real pros? If so, I'd like to make changes.
